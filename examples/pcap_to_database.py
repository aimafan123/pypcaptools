"""将 pcap 文件和可选的 resource JSON 导入 MySQL。

示例：
    python examples/pcap_to_database.py \
        --pcap captures/example.pcap \
        --json captures/example.json \
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="将 pcap 流量导入 trace、flow 和 resource 表。"
    )
    parser.add_argument("--pcap", required=True, help="输入 pcap 文件路径。")
    parser.add_argument(
        "--json",
        default="",
        help="可选的 resource 元数据 JSON 文件，由兼容解码器生成。",
    )
    parser.add_argument(
        "--base-table",
